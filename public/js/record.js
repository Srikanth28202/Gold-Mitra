...
(function () {
  ...
  function jewelleryRowsHTML(items) {
    if (!Array.isArray(items) || !items.length) {
      return `<tr><td colspan="5" style="text-align:center;padding:14px;color:#6B7280">No gold items recorded.</td></tr>`;
    }
    return items
      .map((item, i) => {
        const hasPhoto = !!item.photoData;
        const photoCell = hasPhoto
          ? `<img src="${item.photoData}" alt="Gold item ${i + 1}" style="max-height:54px;max-width:90px;object-fit:contain;border:1px solid #D1D5DB;border-radius:4px;" />`
          : `<span style="font-size:0.78rem;color:#9CA3AF">[ No Image ]</span>`;
  ...
  function render(a) {
    ...
    const jsx = `
      ...
      <div class="gold-paper-form">
        ...
        <!-- 6. SIGNATURES SECTION (Manual / Online Drawn) -->
        <section class="signatures-section">
          <div class="signature-card">
            ...
            <div class="signature-canvas-wrap" style="background:#FFFFFF;border:1px solid #D1D5DB;">
              ${staffSigHTML}
            </div>
